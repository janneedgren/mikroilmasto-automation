...
class NestedGridSolver:
        ...
        if self.coarse.turb_model is not None:
            ...
            if 'epsilon' in fields and fields['epsilon'] is not None:
                interpolators['epsilon'] = RegularGridInterpolator(
                    (y_coarse, x_coarse), fields['epsilon'],
                    method='linear', bounds_error=False, fill_value=None
                )
        ...
        try:
            ...
            if hasattr(obs, 'vertices') and obs.vertices is not None:
                ...
                if is_porous:
                    ...
                    return PolygonBuilding(vertices_list=shifted_vertices, name=name)
    ...
    def _solve_fine_with_nested_bc(self, verbose: bool = True) -> bool:
        ...
        for iteration in range(solver.settings.max_iterations):
            ...
            # Turbulenssin reunaehdot ENNEN ratkaisua
            if solver.turb_model is not None:
                nested_bc.apply_turbulence(solver.turb_model, solver)
            
            # Turbulenssi (ratkaise yhtälöt)
            solver._solve_turbulence(dt)
            
            # Turbulenssin reunaehdot JÄLKEEN ratkaisua
            # (SST:n solve() saattaa ylikirjoittaa reunat)
            if solver.turb_model is not None:
                nested_bc.apply_turbulence(solver.turb_model, solver)
    ...
    def _initialize_fine_from_coarse(self, interpolators: Dict):
        ...
        # Interpoloi virtauskentät (u, v, p) KOKO hilalle
        ...
        self.fine.p = interpolators['p'](points).reshape(self.fine_ny, self.fine_nx)
        
        print(f"    u interpoloitu: min={self.fine.u.min():.2f}, max={self.fine.u.max():.2f}")
        print(f"    v interpoloitu: min={self.fine.v.min():.2f}, max={self.fine.v.max():.2f}")
        
        # Turbulenssi: interpoloi KOKO kenttä karkeasta hilasta
        # Tämä antaa realistisen omega-jakauman (matala kaukana seinistä)
        if hasattr(self.fine, 'turb_model') and self.fine.turb_model is not None:
            tm = self.fine.turb_model
            
            if self.coarse.turb_model is not None and 'k' in interpolators and 'omega' in interpolators:
                # Interpoloi k ja omega KOKO hilalle
                k_interp = interpolators['k'](points).reshape(self.fine_ny, self.fine_nx)
                k_interp = np.maximum(k_interp, tm.const.k_min)
                
                omega_interp = interpolators['omega'](points).reshape(self.fine_ny, self.fine_nx)
                omega_interp = np.maximum(omega_interp, tm.const.omega_min)
                
                # Aseta koko kenttä (ei vain reunat)
                tm.k[:] = k_interp
                tm.omega[:] = omega_interp
                
                # Raportoi
                non_solid = ~self.fine.solid_mask
                print(f"    k interpoloitu KOKO kentälle: mean={tm.k[non_solid].mean():.2e}, max={tm.k[non_solid].max():.2e}")
                print(f"    omega interpoloitu KOKO kentälle: mean={tm.omega[non_solid].mean():.2e}, max={tm.omega[non_solid].max():.2e}")
            else:
...
def _add_obstacles_shifted(ax, obstacles, x_offset: float, y_offset: float):
    ...
    for obs in obstacles:
        ...
        if is_solid:
            # Kiinteä rakennus - harmaa täyttö
            facecolor, edgecolor, alpha, lw = '#404040', 'black', 0.9, 1.5
        else:
            # Kasvillisuusalue - läpikuultava täyttö + selkeä reunaviiva
            veg_type = getattr(obs, 'vegetation_type', None) or getattr(obs, 'obs_type', 'tree_zone')
            if veg_type == 'farmland':
                facecolor, edgecolor, alpha, lw = '#FFE082', '#B8860B', 0.3, 2  # Läpikuultava kellertävä
            else:
                facecolor, edgecolor, alpha, lw = '#90EE90', '#228b22', 0.3, 2  # Läpikuultava vaaleanvihreä
